...
import BrandName from "@/app/(public)/(purchaser)/product/components/BrandName"
import ProductAttributes from "@/app/(public)/(purchaser)/product/components/product-attributes"
import ProductDescription from "@/app/(public)/(purchaser)/product/components/product-description"
import ProductImages from "@/app/(public)/(purchaser)/product/components/product-images"
import ProductOffers from "@/app/(public)/(purchaser)/product/components/product-offers"
import ProductInfo from "@/app/(public)/(purchaser)/product/components/ProductInfo"
import ProductIntroduce from "@/app/(public)/(purchaser)/product/components/ProductIntroduce"
import ProductLowestPriceInfo from "@/app/(public)/(purchaser)/product/components/ProductLowestPriceInfo"
import SameCategories from "@/app/(public)/(purchaser)/product/components/SameCategories"
...
const ProductPage = ({ id, isMobileView }: ProductPageProps) => {
  ...
  queryProduct.data?.product.category.parentsChain.forEach((parent, idx) => {
    breadcrumbJsonLdArray.push({
      ...
      item: {
        "@id": encodeURI(
          `${process.env.NEXT_PUBLIC_API_ENDPOINT}/product/${parent.id}/${parent.title}`
        ),
        name: parent.title
      }
    })
  ...
  breadcrumbJsonLdArray.push({
    ...
    item: {
      "@id": encodeURI(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/products/${queryProduct.data?.product.category.id}/${queryProduct.data?.product.category.title}`
      ),
  ...
  const breadcrumbJsonLd: WithContext<BreadcrumbList> = {
    ...
    itemListElement: [
      ...
      ...(breadcrumbJsonLdArray as ItemList[]),
      {
        ...
        item: {
          "@id": encodeURI(
            `${process.env.NEXT_PUBLIC_API_ENDPOINT}/products/${queryProduct.data?.product.id}/${queryProduct.data?.product.name}`
          ),
  ...
  const productJsonLd: WithContext<ProductSchema> = {
    ...
    url: `${process.env.NEXT_PUBLIC_API_ENDPOINT}/product/${queryProduct.data?.product.id}/${queryProduct.data?.product.name}`,
